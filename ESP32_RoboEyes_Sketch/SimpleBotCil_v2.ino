...
void setup() {
  ...
  Serial.println("SimpleBotCil ESP32 ready!");
...
void loop() {
  ...
  // Check if Mac app sent a command
  if (Serial.available() > 0) {
    ...
    if (command.length() > 0) {
      ...
      if (command == "HAPPY") {
        ...
        eyes.setMood(TIRED);
        Serial.println("-> TIRED (sad)");
      } else if (command == "ANGRY") {
        eyes.setMood(ANGRY);
        Serial.println("-> ANGRY (mad)");
      } else if (command == "DEFAULT") {
        eyes.setMood(DEFAULT);
        Serial.println("-> DEFAULT (neutral)");
      } else {
  ...
  // If app is controlling, check for timeout (return to auto-cycle if no command received)
